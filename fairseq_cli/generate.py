        ...
        for i, sample_id in enumerate(sample['id'].tolist()):
            ...
            for j, hypo in enumerate(hypos[i][:args.nbest]):
                ...
                if not args.quiet:
                    ...
                    print('D-{}\t{}\t{}'.format(sample_id, score, hypo_str), file=output_file)  #deleted
                    print('P-{}\t{}'.format(
                        sample_id,
                        ' '.join(map(
    ...
    if has_target:
        if args.bpe and not args.sacrebleu:
            if args.remove_bpe:
                ...
                logger.warning("If you are using BPE on target side, the BLEU score is computed on BPE tokens, not on proper words.  Use --sacrebleu for standard 13a BLEU tokenization")
